...
class RulesComponent: GKComponent {
    ...
    override func update(deltaTime seconds: TimeInterval) {
        ...
        if timeSinceRulesUpdate < GameplayConfiguration.TaskBot.rulesUpdateWaitDuration {
            return
        }
        
        timeSinceRulesUpdate = 0.0
        
        if let taskBot: TaskBot = entity as? TaskBot, let level: LevelScene = taskBot.component(ofType: RenderComponent.self)?.node.scene as? LevelScene, let entitySnapshot: EntitySnapshot = level.entitySnapshotForEntity(entity: taskBot), !taskBot.isGood {
