...
class FlyingBotPreAttackState: GKState {
    ...
    var animationComponent: AnimationComponent {
        ...
        guard let animationComponent = entity.component(ofType: AnimationComponent.self) else {
            fatalError("A FlyingBotPreAttackState's entity must have an AnimationComponent.")
        }
    ...
    override func update(deltaTime seconds: TimeInterval) {
        ...
        // If the `TaskBot` has been in its "pre-attack" state for long enough, move to the attack state.
