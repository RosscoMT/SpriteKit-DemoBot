...
class MovementComponent: GKComponent {
    ...
    override func update(deltaTime: TimeInterval) {
        ...
        // Check if strafing behavior is enabled. Strafing allows the entity to remain fixed in the direction of the target while the beam is locked.
        ...
        // If an animation is required, and the `AnimationComponent` is running, and the requested animation can be overwritten, update the `AnimationComponent`'s requested animation state.
    ...
    func vectorForBeamTowardsCurrentTarget() -> SIMD2<Float>? {
        ...
        guard let beamComponent = entity?.component(ofType: BeamComponent.self) else {
            return nil
        }
        
        let target = (beamComponent.stateMachine.currentState as? BeamFiringState)?.target
        
        guard let taskBotPosition = target?.component(ofType: RenderComponent.self)?.node.position else {
            return nil
        }
    ...
    func pointForTranslatingNode(node: SKNode, withTranslationalMovement translation: MovementKind, duration: TimeInterval) -> CGPoint? {
        ...
        guard translation.displacement != SIMD2<Float>() else {
            return nil
        }
    ...
    // Determine if the `animationState` can be overwritten. For example, if an `.Attack` animation is being run, we do not want to replace this with any sort of movement animation.
