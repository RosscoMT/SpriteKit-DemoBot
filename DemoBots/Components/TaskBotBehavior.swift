...
class TaskBotBehavior: GKBehavior {
    ...
    static func behaviorAndPathPoints(forAgent agent: GKAgent2D, huntingAgent target: GKAgent2D, pathRadius: Float, inScene scene: LevelScene) -> (behavior: GKBehavior, pathPoints: [CGPoint]) {
        ...
        let behavior: TaskBotBehavior = TaskBotBehavior()
        ...
        let agentsToFlockWith: [GKAgent2D] = scene.entities.compactMap { entity in
            if let taskBot: TaskBot = entity as? TaskBot, !taskBot.isGood && taskBot.agent !== agent && taskBot.distanceToAgent(otherAgent: agent) <= GameplayConfiguration.Flocking.agentSearchDistanceForFlocking {
        ...
        if !agentsToFlockWith.isEmpty {
            ...
            let separationGoal: GKGoal = GKGoal(toSeparateFrom: agentsToFlockWith,
                                                maxDistance: GameplayConfiguration.Flocking.separationRadius,
                                                maxAngle: GameplayConfiguration.Flocking.separationAngle)
            
            behavior.setWeight(GameplayConfiguration.Flocking.separationWeight,
                               for: separationGoal)
            
            let alignmentGoal: GKGoal = GKGoal(toAlignWith: agentsToFlockWith,
                                               maxDistance: GameplayConfiguration.Flocking.alignmentRadius,
                                               maxAngle: GameplayConfiguration.Flocking.alignmentAngle)
            
            behavior.setWeight(GameplayConfiguration.Flocking.alignmentWeight, for: alignmentGoal)
            
            let cohesionGoal: GKGoal = GKGoal(toCohereWith: agentsToFlockWith,
                                              maxDistance: GameplayConfiguration.Flocking.cohesionRadius,
                                              maxAngle: GameplayConfiguration.Flocking.cohesionAngle)
        ...
        let pathPoints: [CGPoint] = behavior.addGoalsToFollowPath(from: agent.position,
                                                                  to: target.position,
    ...
    static func behavior(forAgent agent: GKAgent2D, patrollingPathWithPoints patrolPathPoints: [CGPoint], pathRadius: Float, inScene scene: LevelScene) -> GKBehavior {
        ...
        let pathVectorPoints: [SIMD2<Float>] = patrolPathPoints.map { SIMD2<Float>($0) }
        
        // Create a cyclical (closed) `GKPath` from the provided path points with the requested path radius.
        let path: GKPath = GKPath(points: pathVectorPoints, radius: pathRadius, cyclical: true)
    ...
    */
    private func extrudedObstaclesContaining(point: SIMD2<Float>, inScene scene: LevelScene) -> [GKPolygonObstacle] {
        ...
        //Add a small fudge factor (+5) to the extrusion radius to make sure we're including all obstacles.
        let extrusionRadius: Float = Float(GameplayConfiguration.TaskBot.pathfindingGraphBufferRadius) + 5

        // Return only the polygon obstacles which contain the specified point. Note: This creates a bounding box around the polygon obstacle to check for intersection. This is appropriate for DemoBots, but in your game a more specific check may be necessary.
         
        return scene.polygonObstacles.filter { obstacle in
            
            // Retrieve all vertices for the polygon obstacle.
            let range: Range<Int> = 0..<obstacle.vertexCount
            
            let polygonVertices: Array<SIMD2<Float>> = range.map {
                obstacle.vertex(at: $0)
            }
            ...
            let maxX: Float = polygonVertices.max { $0.x < $1.x }!.x + extrusionRadius
            let maxY: Float = polygonVertices.max { $0.y < $1.y }!.y + extrusionRadius
            
            let minX: Float = polygonVertices.min { $0.x < $1.x }!.x - extrusionRadius
            let minY: Float = polygonVertices.min { $0.y < $1.y }!.y - extrusionRadius
            
            return (point.x > minX && point.x < maxX) && (point.y > minY && point.y < maxY)
        }
    }
    
    // Creates a node on the obstacle graph for the provided point by ignoring the buffer radius of the contacted obstacles. Returns `nil` if a valid connection could not be made.
    private func connectedNode(forPoint point: SIMD2<Float>, onObstacleGraphInScene scene: LevelScene) -> GKGraphNode2D? {
        ...
        let pointNode: GKGraphNode2D = GKGraphNode2D(point: point)
        
        // Try to connect this node to the graph.
        scene.graph.connectUsingObstacles(node: pointNode)

        // Check to see if we were able to connect the node to the graph. If not, this means that the point is inside the buffer zone of an obstacle somewhere in the level. We can't pathfind to a point that is off-graph, so we try to find the nearest point that is on the graph, and pathfind to there instead.
        if pointNode.connectedNodes.isEmpty {
            ...
            let intersectingObstacles: [GKPolygonObstacle] = extrudedObstaclesContaining(point: point, inScene: scene)
        
            // Connect this node to the graph ignoring the buffer radius of any obstacles that the point is currently intersecting.
    ...
    private func addGoalsToFollowPath(from startPoint: SIMD2<Float>, to endPoint: SIMD2<Float>, pathRadius: Float, inScene scene: LevelScene) -> [CGPoint] {
        ...
        guard let startNode: GKGraphNode2D = connectedNode(forPoint: startPoint, onObstacleGraphInScene: scene),
              let endNode: GKGraphNode2D = connectedNode(forPoint: endPoint, onObstacleGraphInScene: scene) else {
            return []
        }
        
        // Remove the "start" and "end" nodes when exiting this scope.
        defer {
            scene.graph.remove([startNode, endNode])
        }
   
        // Find a path between these two nodes and a valid `GKPath` can not be created if fewer than 2 path nodes were found, return.
        guard let pathNodes: [GKGraphNode2D] = scene.graph.findPath(from: startNode, to: endNode) as? [GKGraphNode2D], pathNodes.count > 1 else {
            return []
        }
        ...
        let pathPoints: [CGPoint] = pathNodes.map {
            CGPoint($0.position)
        }
