...
class PlayerBotAppearState: GKState {
    ...
    var animationComponent: AnimationComponent {
        ...
        guard let animationComponent: AnimationComponent = entity.component(ofType: AnimationComponent.self) else {
    ...
    override func didEnter(from previousState: GKState?) {
        ...
        // The `PlayerBot` is about to appear in the level. We use an `SKShader` to provide a "teleport" effect to beam in the `PlayerBot`.
        ...
        let texture: SKTexture = appearTextures[orientationComponent.compassDirection]!
        node.texture = texture
        node.size = PlayerBot.attribute.textureSize

        // Add an `SKShader` to the node to render the "teleport" effect.
        node.shader = PlayerBot.attribute.teleportShader
