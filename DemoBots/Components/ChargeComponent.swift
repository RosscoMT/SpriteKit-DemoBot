...
class ChargeComponent: GKComponent {
    ...
    // A `ChargeBar` used to show the current charge level. The `ChargeBar`'s node is added to the scene when the component's entity is added to a `LevelScene` via `addEntity(_:)`.
    ...
    func loseCharge(chargeToLose: Double) {
        var newCharge: Double = charge - chargeToLose
    ...
    func addCharge(chargeToAdd: Double) {
        var newCharge: Double = charge + chargeToAdd
