...
class TaskBotZappedState: GKState {
    ...
    var animationComponent: AnimationComponent {
        ...
        guard let animationComponent = entity.component(ofType: AnimationComponent.self) else {
            fatalError("A TaskBotZappedState's entity must have an AnimationComponent.")
        }
    ...
    override func update(deltaTime seconds: TimeInterval) {
        ...
        // If the `TaskBot` has become "good" or has been in the current state long enough, re-enter `TaskBotAgentControlledState`.
    ...
    override func isValidNextState(_ stateClass: AnyClass) -> Bool {
        switch stateClass {
            case is TaskBotZappedState.Type:
                ...
                // Reset the elapsed time the `taskBot` has been in `TaskBotZappedState`. This ensures there is a delay from when a `taskBot` stops being zapped to when it becomes agent controlled.
                elapsedTime = 0.0
                return false
