...
class BeamFiringState: GKState {
    ...
    var playerBot: PlayerBot {
        ...
        guard let playerBot: PlayerBot = beamComponent.entity as? PlayerBot else {
            fatalError("A BeamFiringState's beamComponent must be associated with a PlayerBot.")
        }
    ...
    var renderComponent: RenderComponent {
        ...
        guard let renderComponent: RenderComponent = beamComponent.entity?.component(ofType: RenderComponent.self) else {
            fatalError("A BeamFiringState's entity must have a RenderComponent.")
        }
    ...
    override func didEnter(from previousState: GKState?) {
        ...
        if beamComponent.beamNode.parent == nil {
            ...
            let playerBot: PlayerBot = self.playerBot
            
            // The `BeamComponent`'s `BeamNode` is added to the scene at the `.AboveCharacter` level. This ensures it appears above the `PlayerBot` and all `TaskBot`s in the scene.
            guard let scene = renderComponent.node.scene as? LevelScene else { fatalError("The RenderComponent's node must be in a scene.") }

            // Subtract 1 from the beam node's `zPosition` to make sure the beam appears above all characters, but below other elements added to the `AboveCharacters` node.
            beamComponent.beamNode.zPosition = -1.0
            
            let aboveCharactersNode: SKNode = scene.worldLayerNodes[.aboveCharacters]!
            ...
            let xRange: SKRange = SKRange(constantValue: playerBot.antennaOffset.x)
            let yRange: SKRange = SKRange(constantValue: playerBot.antennaOffset.y)
    ...
    override func update(deltaTime seconds: TimeInterval) {
        ...
        if elapsedTime >= GameplayConfiguration.Beam.maximumFireDuration {
            ...
            // The player has been firing the beam for too long. Enter the `BeamCoolingState` to disable firing until the beam has had time to cool down.
    ...
    func updateBeamNode(withDeltaTime seconds: TimeInterval) {
        ...
        if let chargeComponent: ChargeComponent = target?.component(ofType: ChargeComponent.self) {
        ...
        if let currentTarget: TaskBot = target, currentTarget.isGood {
