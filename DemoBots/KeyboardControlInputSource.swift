...
class KeyboardControlInputSource: ControlInputSourceType {
    ...
    var downKeys: Set<Character> = Set<Character>()
    ...
    static let forwardVector: SIMD2<Float> = SIMD2<Float>(x: 1, y: 0)
    static let backwardVector: SIMD2<Float> = SIMD2<Float>(x: -1, y: 0)
    static let clockwiseVector: SIMD2<Float> = SIMD2<Float>(x: 0, y: -1)
    static let counterClockwiseVector: SIMD2<Float> = SIMD2<Float>(x: 0, y: 1)
    ...
    func handleKeyDown(forCharacter character: Character) {
        ...
        if let relativeDisplacement: SIMD2<Float> = relativeDisplacementForCharacter(character) {
    ...
    func handleKeyUp(forCharacter character: Character) {
        ...
        guard downKeys.remove(character) != nil else {
            return
        }
