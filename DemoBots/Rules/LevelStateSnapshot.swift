...
// Stores a snapshot of the state of a level and all of its entities (`PlayerBot`s and `TaskBot`s) at a certain point in time.
    ...
    init(scene: LevelScene) {
        ...
        */
        for sourceEntity in scene.entities {
            ...
            guard let sourceIndex: Set<GKEntity>.Index = scene.entities.firstIndex(of: sourceEntity) else {
                continue
            }

            // Retrieve the `GKAgent` for the source entity.
            let sourceAgent: GKAgent2D = agentForEntity(entity: sourceEntity)
            ...
            for targetEntity in scene.entities[scene.entities.index(after: sourceIndex) ..< scene.entities.endIndex] {
                ...
                let targetAgent: GKAgent2D = agentForEntity(entity: targetEntity)
                ...
                let distance: Float = hypotf(dx, dy)
        ...
        let (goodTaskBots, badTaskBots) = scene.entities.reduce(([], [])) {
            ...
            guard let thisTaskBot = thisEntity as? TaskBot else {
        ...
        for entity in scene.entities {
            let entitySnapshot: EntitySnapshot = EntitySnapshot(badBotPercentage: badBotPercentage, proximityFactor: scene.levelConfiguration.proximityFactor, entityDistances: entityDistances[entity]!)
