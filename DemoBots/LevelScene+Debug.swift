...
extension LevelScene {
    ...
    func debugDrawingEnabledDidChange() {
        ...
        // Turn on debug drawing for every obstacle in the level, to show their pathfinding buffer radius.
        ...
        for componentSystem in componentSystems {
            ...
            guard componentSystem.componentClass is BeamComponent.Type else {
                continue
            }
    ...
    func drawGraph() {
        ...
        for node in graph.nodes as! [GKGraphNode2D] {
            for destination in node.connectedNodes as! [GKGraphNode2D] {
                var points: [CGPoint] = [CGPoint(node.position), CGPoint(destination.position)]
                let shapeNode: SKShapeNode = SKShapeNode(points: &points, count: 2)
...
extension SKSpriteNode {
    ...
    var debugDrawingEnabled: Bool {
        set {
            ...
            if newValue == true {
                let bufferRadius: CGFloat = CGFloat(GameplayConfiguration.TaskBot.pathfindingGraphBufferRadius)
                let bufferFrame: CGRect = frame.insetBy(dx: -bufferRadius, dy: -bufferRadius)
                let bufferedShape: SKShapeNode = SKShapeNode(rectOf: bufferFrame.size)
                ...
                guard let debugBufferShape: SKNode = childNode(withName: debugBufferShapeName) else {
                    return
                }
