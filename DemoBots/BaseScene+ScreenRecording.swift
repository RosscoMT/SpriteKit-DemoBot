...
extension BaseScene: RPPreviewViewControllerDelegate, RPScreenRecorderDelegate {
    ...
    func startScreenRecording() {
        ...
        guard screenRecordingToggleEnabled else {
            return
        }
        
        let sharedRecorder: RPScreenRecorder = RPScreenRecorder.shared()
    ...
    func stopScreenRecording(withHandler handler:@escaping (() -> Void)) {
        ...
        sharedRecorder.stopRecording { previewViewController, error in
            ...
            if let previewViewController: RPPreviewViewController = previewViewController {
                // Set delegate to handle view controller dismissal.
                previewViewController.previewControllerDelegate = self
                
                // Keep a reference to the `previewViewController` to present when the user presses on preview button.
    ...
    func showScreenRecordingAlert(message: String) {
        ...
        let alertController: UIAlertController = UIAlertController(title: "ReplayKit Error", message: message, preferredStyle: .alert)
        ...
        // `ReplayKit` event handlers may be called on a background queue. Ensure this alert is presented on the main queue.
