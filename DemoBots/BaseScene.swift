...
class BaseScene: SKScene, GameInputDelegate, ControlInputSourceGameStateDelegate, ButtonNodeResponderType {
    ...
    var focusChangesEnabled: Bool = false
    ...
    var overlay: SceneOverlay? {
        ...
        didSet {
            ...
            if let overlay: SceneOverlay = overlay, let camera: SKCameraNode = camera {
    ...
    func controlInputSource(_ controlInputSource: ControlInputSourceType, didSpecifyDirection direction: ControlInputDirection) {
        ...
        guard focusChangesEnabled else {
            return
        }
        ...
        // Create the button focus graph to ensure it represents what's being displayed and includes any buttons that may have been added to the scene.
        createButtonFocusGraph()
        
        // Update the focused button to the neighbor of the currently focused button that lies in the direction the input source specified.
        if let currentFocusedButton: ButtonNode = focusedButton {
            
            if let newFocusButton: ButtonNode = currentFocusedButton.focusableNeighbors[direction] {
                focusedButton = newFocusButton
                
                // Reset `focusChangesEnabled` after a 0.2 second delay to ensure buttons are traversed at a reasonable rate even in the presence of constant input.
                focusChangesEnabled = false
                
                DispatchQueue.main.asyncAfter(deadline: .now() + DispatchTimeInterval.microseconds(200)) {
                    self.focusChangesEnabled = true
                    
                    // Resetting the control state will check the current values and possibly call `controlInputSource(_: didSpecifyDirection:)` again.
