...
class BeamNode: SKNode, ResourceLoadableType {
    ...
    static var dotTextureSize: CGSize = CGSize(width: 30.0, height: 30.0)

    static let animationActionKey: String = "Animation"

    static let lineNodeTemplate: SKSpriteNode = {
       
        guard let templateScene: SKScene = SKScene(fileNamed: "BeamLine.sks"), let beamLine: SKSpriteNode = templateScene.childNode(withName: .beamLine) as? SKSpriteNode else {
            fatalError()
        }
    ...
    var runningNodeAnimations: [SKNode: SKAction] = [SKNode: SKAction]()
    ...
    override init() {
        ...
        arcPath.addArc(center: center,
                       radius: GameplayConfiguration.Beam.arcLength,
                       startAngle: GameplayConfiguration.Beam.arcAngle * 0.5,
                       endAngle: GameplayConfiguration.Beam.arcAngle * -0.5,
                       clockwise: true)
    ...
    func update(withBeamState state: GKState, source: PlayerBot, target: TaskBot? = nil) {
        ...
        if let target: TaskBot = target, let targetNode: SKNode = target.component(ofType: RenderComponent.self)?.node, destinationNode.constraints?.first?.referenceNode != targetNode {
                
                let xRange: SKRange = SKRange(constantValue: target.beamTargetOffset.x)
                let yRange: SKRange = SKRange(constantValue: target.beamTargetOffset.y)
        ...
        switch state {
            ...
            case is BeamFiringState:
                ...
                // If there is no `lineNode`, create one from the template node. Adding a new copy of the template will ensure the actions are re-started when the beam starts being fired.
                if lineNode == nil {
    ...
    static func loadResources() async {
        ...
        let beamAtlasNames: [String] = ["BeamDot", "BeamCharging"]
        
        // Preload all of the texture atlases for `BeamNode`. This improves the overall loading speed of the animation cycles for the beam.
        do {
            let beamAtlases: [SKTextureAtlas] = try await SKTextureAtlas.preloadTextureAtlasesNamed(beamAtlasNames)
            
            let beamDotAction: SKAction = AnimationComponent.actionForAllTexturesInAtlas(atlas: beamAtlases[0])
