...
struct ColliderType: OptionSet, Hashable, CustomDebugStringConvertible {
    ...
    var collisionMask: UInt32 {
        ...
        let mask: ColliderType? = ColliderType.definedCollisions[self]?.reduce(ColliderType()) { initial, colliderType in
    ...
    var contactMask: UInt32 {
        ...
        let mask: ColliderType? = ColliderType.requestedContactNotifications[self]?.reduce(ColliderType()) { initial, colliderType in
            return initial.union(colliderType)
        }
