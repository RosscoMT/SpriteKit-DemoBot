...
struct SceneMetadata {
    ...
    var onDemandResourcesTags: Set<String> = []
    
    // A flag indicating whether the scene requires on demand resources to load.
    var requiresOnDemandResources: Bool {
        #if os(OSX)
        // OS X does not use on demand resources, so resources will always be available on disk.
        return false
        #else
        // Check for on demand resources, not all scenes have resources that need to be downloaded.
    ...
    init(sceneConfiguration: SceneConfiguration) {
        ...
        if let tags: [String] = sceneConfiguration.onDemandResourcesTags {
            onDemandResourcesTags = Set(tags)
            
            // The tags are also used to determine which enemies need their resources to be preloaded for a `LevelScene`.
        ...
        // We will always need the `PlayerBot` and the `BeamNode` if the scene is a `LevelScene`, so add these `ResourceLoadableType`s by default.
