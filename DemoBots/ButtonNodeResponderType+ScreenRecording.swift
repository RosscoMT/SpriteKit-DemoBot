...
// Extend `ButtonNodeResponderType` to add methods for screen recording with ReplayKit. The type constraint ensures that only types that are `BaseScene` instances will  get this additional functionality.
...
extension ButtonNodeResponderType where Self: BaseScene {
    ...
    func displayRecordedContent() {
        ...
        guard let previewViewController: RPPreviewViewController = previewViewController else {
